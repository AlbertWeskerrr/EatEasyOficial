      ...
      <div className="lg:w-1/2 bg-easyeat-light flex flex-col items-center justify-center p-6 lg:p-16">
        <div className="w-full max-w-md space-y-6 animate-fade-in">
          ...
          <div className="flex flex-col items-center justify-center gap-2 text-sm text-gray-500">
            ...
            {/* DEBUG INFO - MANTENHA PARA SUPORTE */}
            <div className="mt-4 p-2 bg-gray-100 rounded text-[10px] font-mono opacity-30 hover:opacity-100 transition-opacity">
              <p>URL: {import.meta.env.VITE_SUPABASE_URL || 'NF'}</p>
              <p>KEY: {import.meta.env.VITE_SUPABASE_ANON_KEY?.substring(0, 10)}...{import.meta.env.VITE_SUPABASE_ANON_KEY?.substring(import.meta.env.VITE_SUPABASE_ANON_KEY?.length - 5)}</p>
            </div>
          </div>
        </div>
      </div>
